...
2. **John Hughes — "How to Specify It!" (2019, paper + talk).** The decision-procedure
   form of property-based testing: five property styles (invariant, postcondition,
   metamorphic, inductive, model-based) measured against seeded bugs on one worked
   example. Companions: "Testing the Hard Stuff and Staying Sane" (stateful/race
   evidence); Wlaschin's pattern-name taxonomy (roundtrip, oracle, idempotence).
   Partial anchors already in the ledger: the bar-map inertness property (a property
   certifies the artifact, never the wiring — it proved equivalence while hiding
   non-arrival) and the per-platform parser parity suites (a cross-implementation
   property in production). Agent-era hook for the digest: example tests written by the
   model that wrote the code inherit its misconceptions; a property plus a generator
...
4. **Kleppmann (weak-isolation talks, e.g. "Transactions: myths, surprises and
...
5. **Hyrum's Law.** Anything you expose gets depended on; Ousterhout endorses it in the
...
6. **Richard Cook — "How Complex Systems Fail."** The operations axis the corpus lacks
